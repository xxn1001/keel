...
keel 是**给服务器做的**:一台长期在线、尽量不重装、升级要能原子回退的机器。
「不可变基座 + A/B 双槽 + 系统与数据分离」这套设计的价值全在这里 ——
换版本 = 重启一次,起不来自己退回上一个好版本,而不是「升级失败 → 现场抢修 → 重装」。
未来它还要当**虚拟化宿主**(GPU 直通给 VM;宿主不需要显卡驱动,所以 `server` 变体比 `desktop` 更小)。

**笔记本在这里是「顺带兼任」**:项目所有者目前还没有服务器,于是先把它装到自己天天用的笔记本上 ——
用得越早、bug 暴露得越早;等真上服务器时问题已经磨平。
所以 `main` 保持**无桌面的最小系统**(= 服务器形态),`desktop` profile 只是笔记本兼任时的可选加成,
不是主线。
