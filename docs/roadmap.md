...
| **v1.2** | 安全 | **更新签名**(1.1)→ **Secure Boot**(1.2,含 UKI 签名、自己的密钥库、解封 `pcrlock`);initrd 冻结修复(3.0);**待定**:`/etc` 三方合并(2.10 —— 业主希望排这里,评估建议改 v2.0,见该节) | —— |
| **v2.0** | 结构与可信 | **迁移执行器**(2.8,先做,它是下面一切的前提)→ **`/data` 加密 + TPM 封印**(1.3)+ **dm-verity**(1.4)+ **早期启动重排**(把 `/etc` overlay 提到 initrd,见 D19 方案 A)→ 可选:`systemd-sysupdate` 底层(2.5)、`cache` 分区(2.4)、`server` profile(2.2)、`/etc` 三方合并(2.10,建议排这里) | —— |
...
## 2.10 评估:用 `/usr/etc` + `/data/etc` 合并取代 `/etc` overlayfs(2026-09-26 评估,**未实施**)
...
**排期建议**:⚠ 业主希望排 **v1.2**;评估建议排 **v2.0** —— ① roadmap §0 的原则是"结构最后做";
② 它**依赖** v2.0 已排的两件事(2.8 迁移执行器、D19 方案 A 早期启动重排)。
**更划算的先后**:先把 **D19 方案 A(把 `/etc` 提到 initrd 里挂)** 做掉 —— 一次消掉
#24/#29/#37/#61 四条,且**不需要**重写 `/etc` 结构;`/usr/etc` + 三方合并在那之后作为
"去掉 overlay"的后续(边际收益小得多)。**版本待业主拍板。**
