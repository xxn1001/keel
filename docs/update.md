...
> **③ 之前还有两道"动分区之前"的检查(v1.1 ②)**:ESP 必须**已挂上**、而且**可用空间够**
> (判据:放得下 `新 UKI ×2 + 50 MiB`,够两个槽 + 一点余量)。任何一条不满足就**拒绝往下走**,
> 而不是先写根分区 —— 因为"根换好了、新 UKI 却没写进去"就是**内核与根不配对**(不变量 3),
> 而且失败点离原因很远。空间不够时会提示 `sudo os-rescue --clean-esp`。
> `stage` 同时会清掉**所有**槽的 `.failed` / `.bad` 墓碑(它们不参与启动,只吃 ESP 空间)。
