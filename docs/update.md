...
| 步骤 | 做什么 | 失败时 |
|---|---|---|
| `os-status` | 当前槽/版本/内核、`/Volume` 用量、schema 版本、pending 状态、上次更新结果、槽位占用(§9) | — |
| `os-update check` | 查询更新源里有没有比当前更新的版本 | 先查网络,再查 `/Volume/keel/config` 里的 URL(§6) |
| `os-update fetch` | 下载到 `/Volume/ota/<ver>/`,校验 sha256、签名(v1 只留接口)、schema 兼容性(§8) | 什么都没写进槽,重试即可 |
...
失败的载荷不会自动删:留在 `/Volume/ota/` 里(§4.2),查清原因后用 `os-update gc` 清理
(保留最近 2 个版本 + 当前,§8)。

## 4. `/Volume` 的 schema 迁移约定(最重要的一节)

`/Volume` 是 A/B 两个槽**共享**的可写分区。回滚时旧系统挂载的是**同一份** `/Volume`,
...
| **必须 bump 版本** | 动了 `/Volume` 布局就要 bump `/Volume/keel/schema-version`,并在下面那张表里记一行;`fetch` 阶段会做 schema 兼容性校验(§8) |
...
3. 每次 schema 变更后**做一次完整回滚演练**:stage → 重启 → 确认新系统 → `os-update rollback` →
   重启 → 确认旧系统在这份被改过的 `/Volume` 上正常工作。**没演练过的 schema 变更不算完成。**
...
| 风险 | 症状 | 处理 |
|---|---|---|
| **`/etc` overlay 的 upper 是共享的**(R2) | 新版本往 `/etc` 写了旧版本不认识的配置(新 drop-in、新格式),回滚后旧系统行为异常 | `sudo os-rescue --reset-etc` 兜底:清空 overlay upper,下次启动重新从镜像播种。**它会丢掉你在 `/etc` 里的本机配置**(ssh 主机密钥、machine-id、账号、网络配置)——先备份到 `/Volume/home/...` 再执行 |
...
更新源 URL 在 `/Volume/keel/config` 里(§4.2、§8),支持三种形式:

| 形式 | 例子 | 用途 |
|---|---|---|
| `https://` | `https://example.invalid/keel/` | 正式发布 |
| `file://` | `file:///Volume/ota/local/` | 本机/离线目录,自测 |
...
| 2 | 有 `/Volume` schema 变更? | 先在 manifest 里写声明式迁移步骤、bump `schema-version`,并在本文档 §4 的表里加一行 —— **这步不能跳过** |
...
| 5 | 上传整个目录 | 放到 `/Volume/keel/config` 指向的更新源;版本号由 `mkosi.version` + 构建时 `-B` 自动 bump(§6) |
